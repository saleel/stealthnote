...
async function postLike(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // Validate pubkey
    const { data: membershipData } = await supabase.from('memberships').select().eq('pubkey', pubkey).single();
    ...
    if (!like && existingLike) {
      // Unlike
      await Promise.all([
        supabase.from('likes').delete().eq('message_id', messageId).eq('pubkey', pubkey),
